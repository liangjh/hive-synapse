    ...
    def test_full_mvp_workflow(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for command in ["classify", "compact", "propose"]:
                result = run_hive("import", command, import_id, "--workspace", str(workspace), "--json")
                self.assertEqual(result.returncode, 0, command + result.stdout + result.stderr)
            sweep = run_hive("promote", "sweep", "--workspace", str(workspace), "--create-proposals", "--json")
            self.assertEqual(sweep.returncode, 0, sweep.stderr)
            proposal_id = json.loads(sweep.stdout)["created_proposals"][0]["id"]
            review = run_hive("promote", "review", proposal_id, "--workspace", str(workspace), "--decision", "approved", "--actor", "steward:engineering", "--rationale", "e2e", "--json")
            self.assertEqual(review.returncode, 0, review.stderr)
            apply = run_hive("promote", "apply", proposal_id, "--workspace", str(workspace), "--actor", "steward:engineering", "--json")
            ...
            migration = run_hive("upgrade", "migration-dry-run", "001_runtime_markers", "--workspace", str(workspace), "--json")
