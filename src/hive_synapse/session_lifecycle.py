...
def _bootstrap_markdown(adapter: str, record: dict[str, Any], context_dir: Path, outbox_dir: Path) -> str:
...
- Actor: `{record['actor']}`
- Effective node: `{record['effective_node']}`
- Role: `{record['role']}`
- Sign-in: `{record['id']}`
- Personal memory home: `{record['personal_memory_home']}`
...
def _soul_text(record: dict[str, Any], purpose: str | None) -> str:
    purpose_text = purpose or "No explicit session purpose supplied."
    return f"""# SOUL
...
Use the context snapshot for this run. If context appears stale, stop and ask for or run `hive actor refresh {record['id']}` before relying on older facts.
"""
...
def _copy_adapter_aliases(adapter: str, mount_path: Path, bootstrap_text: str) -> list[Path]:
    ...
    if adapter == "claude":
        ...
        _write_generated(
            tools_path,
            "# TOOLS\n\nUse the `hive` CLI for session finish, imports, proposals, compaction, validation, and audit.\n",
        )
        _write_generated(bootstrap_path, bootstrap_text)
