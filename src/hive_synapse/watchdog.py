...
def watchdog_report(root: Path, *, enqueue: bool | None = None, actor: str = "system:watchdog") -> dict[str, Any]:
    ...
    for marker in dirty:
        ...
        if target not in pending_text:
            findings.append({"code": "dirty_context_without_job", "target": target, "path": str(marker.relative_to(paths.root))})
            if enqueue:
                enqueue_job(paths.root, "context_rebuild", str(target), reason="watchdog dirty context", actor=actor)
    for path in paths.root.rglob("*"):
        lower = path.name.lower()
        if any(marker in lower for marker in ["conflict", "conflicted copy", "sync conflict"]):
            if "memory/conflicts" in str(path):
                continue
            findings.append({"code": "sync_conflict_file", "path": str(path.relative_to(paths.root))})
